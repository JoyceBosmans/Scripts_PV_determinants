...
df_allgridcells				<- '/vol/milkunB/jbosmans/PV_Konrad_Afr/df_Afr_allgridcells.csv'
dir_Konrad 					<- '/vol/milkunarc2/kmielke/pv' 
dir_Joyce					<- '/vol/milkunB/jbosmans/PV_Konrad_Afr'
...
model_performance <- read.csv(file.path(dir_Konrad,'/results/waic_AfrRev.txt'), sep = ' ', header = FALSE)
...
CRS.new <- CRS("ESRI:102024") # Lambert projection # different projection due to error
...
foreach(i = 1:nRuns) %dopar% {
    ...
    formula <- y ~ 1 + travel_log + slope_log + road_log + grid_log + rsds + Agriculture + Short_natural + Bare +  f(SOVEREIGN1, model = "iid") + f(spatial.field, model = spde)
    ...
    print('i')
    # save the model
    save(model, file = paste0(dir_Joyce, "/models/", toString(i), ".Rda"))
    ...
    save(df_full_temp, file = paste0(dir_Joyce, "/predictions/", toString(i), ".Rda"))
