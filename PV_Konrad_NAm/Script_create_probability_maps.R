...
df_NAm_allgridcells				<- 'df_NAm_allgridcells.csv'
...
if(file.exists(df_NAm_allgridcells)) {
  print('loading dataframe with all grid cells')
  df_full <- read.csv(df_NAm_allgridcells)
} else {
  print('updating dataframe with grid_km and storing in this dir')
  ### dataframe with all grid cells already created in ../PV_determinants_NAm/Script_create_probability_maps.R
  df_full <- read.csv(file.path('../PV_determinants_NAm/df_NAm_allgridcells.csv'))
  ...
  # store csv in this dir rather than redoing grid distance addition. Notice a small number of points don't have grid_km values (islands / coasts, 590 out of 23310688)
  df_full <- df_full[complete.cases(df_full$grid_km),]	
  fwrite(df_full,df_NAm_allgridcells)
}
...
# log-transform a subset of variables and normalize all (from /vol/milkunarc2/kmielke/pv/code/preprocessing_NAm.R)
