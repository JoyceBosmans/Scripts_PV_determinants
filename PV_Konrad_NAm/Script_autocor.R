...
# for NAm find the model without road-dist, see Script_find_best_model.R!
...
# NAmope: 'V1 = travel_log, V2 = slope_log, V3 = elev_log, V4 = rsds, V5 = Agricultural, V6 = Short_natural, V7 = Wetland, V8 = Bare'
# create stack of data used for fitting
#stk.fit <- inla.stack(data=list(y = as.integer(trainFrame[,1], n = length(trainFrame[,1]))),
stk.fit <- stk.fit <- inla.stack(data=list(y = as.integer(trainFrame$PA_bool, n = length(trainFrame$PA_bool))),
...
stk.pred <- stk.fit <- inla.stack(data=list(y = as.integer(testFrame$PA_bool, n = length(testFrame$PA_bool))),
                            effects=list(c(list(Intercept=1), iset),
...
save(moranI,file='moranI_NAm.Rda')
