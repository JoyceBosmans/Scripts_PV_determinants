...
#~ # updating predictor values road_dist, slope and elev. Get from global raster using existing df with P & A
...
#~ df_Eur <- read_excel('/vol/milkunB/jbosmans/SolarWindPotentials_Eur/df_PA_Eur_country_281021.xlsx')

### reading and preparing input
### ----------------------------------------------------------------------------------------------------
polygon_region <- st_read('/vol/milkunB/jbosmans/SolarWindPotentials_Eur/polygon_europe.shp')	# change for other regions!
outputfilename <- 'df_PA_Eur_country_060622.xlsx'												# change for other regions!
region         <- c('Andorra', 'Austria', 'Belgium', 'Denmark', 'Faeroe Islands', 'Finland', 'France', 'Germany', 'Gibraltar', 'Greece', 'Iceland', 'Ireland', 'Italy', 'Liechtenstein', 'Luxembourg', 'Malta', 'Monaco', 'Netherlands', 'Portugal',' San Marino', 'Spain', 'Sweden', 'Switzerland', 'United Kingdom', 'Holy See (Vatican City)', 'Guernsey','Jersey', 'Svalbard','Belarus', 'Republic of Moldova', 'Ukraine', 'Albania', 'Bosnia and Herzegovina', 'Bulgaria', 'Croatia', 'Cyprus', 'Czech Republic', 'Estonia', 'Hungary', 'Latvia', 'Lithuania', 'Poland', 'Romania', 'Montenegro','Serbia', 'Slovak Republic', 'Slovenia', 'Turkey', 'Macedonia')  # change for other regions!
...
#~ pred_stack  <- stack(pred_stack,pred_stack_upd)
...
outputfilename_stack <- 'predictor_stack_Eur_261121.gri'
...
print(paste('CHECK: dataframe of absences contains',nrow(abs_df),'rows / locations, should be', nr_absences))

pdf(file='Plot_presences_absences_spatial.pdf')
ggplot() + theme_minimal() + 
geom_point(data=abs_df,aes(x=x,y=y),size=.2) +  geom_point(data=wiki_sel_lonlat, aes(x=lon_wiki, y=lat_wiki),color='red',size=.5) + labs(title = paste0("Utility-scale PV presences (red, ", nr_presences,") and absences (black, ", nr_absences,")"))
dev.off()
...
df_country <- subset(df_countryEEZ,select=c("PA","PA_bool","ID", "road_dist", "grid", "access", "slope", "elev", "protect", "rsds",	 "Agriculture", "Forest", "Short_natural", "Wetland", "Urban", "Bare", "Water", "available_area", "lat_orig", "lon_orig", "SOVEREIGN1", "POL_TYPE", "Y_1","x_1","AREA_KM2"))
