...
df_PA <- read_xlsx('df_PA_Eur_country_060622.xlsx')
...
par(mfrow=c(6,2),mar=c(2,3,1,1),cex=1.1)
hist(df_pred$elev_shift,breaks=100,main="Elevation")
hist(df_pred$elev_log,breaks=100,main=expression("Log"['10']*" Elevation"))
hist(df_pred$road_dist,breaks=100,main="Road distance")
hist(df_pred$road_dist_log,breaks=100,main=expression("Log"['10']*" Road distance"))
hist(df_pred$slope,breaks=100,main="Slope")
hist(df_pred$slope_log,breaks=100,main=expression("Log"['10']*" Slope"))
hist(df_pred2$grid_proximity,breaks=100,main="Grid proximity")
hist(df_pred2$grid_log,breaks=100,main=expression("Log"['10']*" Grid proximity"))
hist(df_pred2$travel_time,breaks=100,main="Travel time")
hist(df_pred2$travel_log,breaks=100,main=expression("Log"['10']*" Travel time"))
hist(df_PA$irradiance,breaks=100,main="Irradiance")
hist(irr_log,breaks=100,main=expression("Log"['10']*" Irradiance"))
dev.off()
...
par(mfrow=c(4,2),mar=c(2,3,1,1),cex=1.1)
