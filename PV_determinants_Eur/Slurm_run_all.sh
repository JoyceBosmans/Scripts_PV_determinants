...
wait
Rscript Script_PA_predictors_add.R > output_PA_predictors_add.txt &
wait
Rscript Script_distance_to_other_parks.R &	# not needed if using INLA, was used for testing glmer with autocovariate
wait
...
### The modelling part should be updated if moving to the INLA methods (by Konrad?)
#~ date +"%Y-%m-%d %T"
