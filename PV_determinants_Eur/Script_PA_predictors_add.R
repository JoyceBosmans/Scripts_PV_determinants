...
df_PA <- read_excel('/vol/milkunB/jbosmans/PV_determinants_Eur/df_PA_Eur_country_060622.xlsx')
outputfilename <- 'df_PA_Eur_country_060622.xlsx'
...
names(df_PA)[names(df_PA) == "grid"]      <- "grid_old"	# needed if rerun *predictors.R

df_PA <- cbind(df_PA,values_PA)
nrow(df_PA)
df_PA = df_PA[,!(names(df_PA) %in% c('geometry'))]		#2022 not needed?
