...
### Plots for Oceania
###-----------------------------------------------------------------------------------------------------------
Oc_PV_COE_PLh <- mask(global_PV_COE_PLh,polygon_region_Oc)			# select Asia and shift longitudes as above
x1              <- crop(Oc_PV_COE_PLh, extent(-180, 0, -90, 90))
x2              <- crop(Oc_PV_COE_PLh, extent(0, 180, -90, 90))   
extent(x1)      <- c(180, 360, -90, 90)
Oc_PV_COE_PLh   <- merge(x1, x2)
Oc_PV_COE_PLh   <- crop(Oc_PV_COE_PLh,extent(100,200,-50,0))

df_Oc_PV_COE   <- as.data.frame(Oc_PV_COE_PLh,xy=TRUE,na.rm=TRUE)
colnames(df_Oc_PV_COE) <- c('x','y','PV_COE_PLh')
plot_Oc_PV_COE <- ggplot() + geom_sf(data=polygon_region_Oc,size = 0.2,fill='#666666') + coord_sf(xlim = c(20, 200), ylim = c(-50, 81), expand = FALSE) + geom_tile(data=df_Oc_PV_COE, aes(x=x, y=y, fill=PV_COE_PLh), alpha=0.8) +  theme(
    panel.background = element_blank(),axis.ticks = element_blank(),axis.title = element_blank(),axis.text = element_blank(),  legend.key.width = unit(0.8,'line'), legend.key.height = unit(3,'line'), legend.margin=margin(-1,-1,-1,-15),legend.text=element_text(size=14), legend.title=element_blank()) + scale_fill_gradientn(colors = color_scheme_COE,breaks = br_COE,labels = lab_COE,limits = lim_COE,na.value = 'transparent') 
