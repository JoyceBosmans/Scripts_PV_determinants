...
### ----------------------------------------------------------------------------------------------------
polygon_region <- st_read('polygon_Asia.shp')					# change for other regions!
outputfilename <- 'df_PA_Asia_country_060622.xlsx'				# change for other regions!
region         <- c('Russia','Kazakhstan','Kyrgyzstan','Tajikistan','Turkmenistan','Uzbekistan','Armenia','Azerbaijan','Georgia','Bahrain','Iran','Iraq','Israel','Jordan','Kuwait','Lebanon','Oman','Qatar','Saudi Arabia','Syrian Arab Republic','United Arab Emirates','Yemen', 'Brunei Darussalam', 'Cambodia', 'Lao People\'s Democratic Republic', 'Malaysia', 'Philippines', 'Singapore', 'Myanmar','Vietnam', 'Thailand','Timor-Leste', 'Indonesia', 'Papua New Guinea','India', 'Afghanistan', 'Bangladesh', 'Bhutan', 'Maldives', 'Nepal', 'Pakistan', 'Sri Lanka','China','Hong Kong','Macau','Mongolia','Taiwan','Korea, Democratic People\'s Republic of','South Korea','Japan','Korea, Republic of')  	  # change for other regions!
...
outputfilename_raster <- 'predictor_stack_Asia_261121.gri'
#~ writeRaster(predictors,filename=outputfilename_raster,overwrite=TRUE,format='raster')

#~ print('stop script for now after creating outputfile raster')
#~ stop()

predictors <- stack(outputfilename_raster)
...
nr_absences      <- 10*nr_presences/0.465
abs_weight       <- dismo::randomPoints(absence_raster,nr_absences)
abs_df           <- as.data.frame(abs_weight)
print(paste('CHECK: dataframe of absences contains',nrow(abs_df),'rows / locations, should be', nr_presences*10))

pdf(file='Plot_presences_absences_spatial.pdf')
ggplot() + theme_minimal() + 
geom_point(data=abs_df,aes(x=x,y=y),size=.5) +  geom_point(data=wiki_sel_lonlat, aes(x=lon_wiki, y=lat_wiki),color='red',size=.5) + labs(title = paste0("Utility-scale PV presences (red, ", nr_presences,") and absences (black, ", nrow(abs_df),")")) + coord_sf(xlim = c(34, 190))
dev.off()
