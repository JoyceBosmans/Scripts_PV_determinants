...
#~ pred_stack     <- stack('predictor_stack_Asia_261121.gri')
#~ polygon_region <- st_read('polygon_Asia.shp')	
...
outputfilename <- 'df_Asia_allgridcells.csv'
outputfilename_raster           <- 'Probability_map_Asia_.gri'
...
#~ country_data    <- st_read('/vol/milkunB/jbosmans/SolarWindPotentials_Asia/Asia_data.gpkg')		#created in Script_create_country_polygons.R
...
df_country$SOVEREIGN1[df_country$SOVEREIGN1 %in% c('Andaman and Nicobar')] <- 'India'
df_country$SOVEREIGN1[df_country$SOVEREIGN1 %in% c('Kuril Islands')] <- 'Russia'
df_country$SOVEREIGN1[df_country$SOVEREIGN1 %in% c('Oecusse')] <- 'East Timor'
