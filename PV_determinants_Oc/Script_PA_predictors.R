...
### ----------------------------------------------------------------------------------------------------
polygon_region <- st_read('polygon_Oceania.shp')			# change for other regions!
outputfilename <- 'df_PA_Oc_country.xlsx'					# change for other regions!
...
outputfilename_stack <- 'predictor_stack_Oc.gri'
...
# somehow randomPoints doesnt work for Oc (generated random points = 0.0823529411764706 times requested number; Use a higher value for tryf) so use sample instead (*9 to get right amount):
abs     <- sampleRandom(absence_raster,nr_absences*9,xy=TRUE)
...
abs_df      <- subset(abs_df,select=c('x','y'))
