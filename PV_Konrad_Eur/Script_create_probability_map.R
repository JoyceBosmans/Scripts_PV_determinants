...
A_full    <- inla.spde.make.A(mesh=mesh_full,loc=as.matrix(coords_full))
...
# use df_full as test data and create stack used for predicting. Using iset_full and A_full
testFrame <- df_full
stk.pred  <- inla.stack(data=list(y = rep(NA, length(testFrame[,1])), n = length(testFrame[,1])),
                            effects=list(c(list(Intercept=1), iset_full),
...
formula <- y ~ 1 + travel_log + slope_log + elev_log + protect + rsds + Agriculture + Short_natural + Urban + f(SOVEREIGN1, model = "iid") + f(spatial.field, model = spde_full)

# build the model - this overwrites the model loaded from the Rda file? (best model found through Script_find_best_model.R?)
model <-inla(formula,
                data=inla.stack.data(stk.full,spde=spde_full), # data plus spde
