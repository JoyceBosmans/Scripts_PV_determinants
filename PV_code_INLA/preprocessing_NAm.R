...
df_PA <- read_xlsx('../data/xlsx/df_PA_NAm_country_060622.xlsx')

# coordinate transformation
d <- data.frame(lon=df_PA$lon_orig, lat=df_PA$lat_orig)
coordinates(d) <- c("lon", "lat")
proj4string(d) <- CRS("+init=epsg:4326") # WGS 84
CRS.new <- CRS("+init=esri:102009") # Lambert projection
...
# save dataframe
save(df_PA, file = '../data/rda/df_PA_NAm_country_060622_proprocessed.Rda')
