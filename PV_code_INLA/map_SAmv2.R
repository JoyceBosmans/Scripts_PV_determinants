...
df_allgridcells				<- '/vol/milkunarc2/kmielke/pv/data/rda/df_full_SAm_country_060622_proprocessed.Rda'
...
foreach(i = 1:nRuns) %dopar% {
    ...
    if (i * perRun < nrow(df_full)) {
        df_full_temp <- df_full_temp[(i*perRun):((i+1)*perRun),]
    } else {
        df_full_temp <- df_full_temp[(i*perRun):(nrow(df_full)),]
    }
