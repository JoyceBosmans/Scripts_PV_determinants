...
load(file = file.path(dir_Konrad, '/data/rda/var_combs/combinations_SAm.Rda'))
...
foreach(i = 1:nrow(combs)) %dopar% {
    ...
    # write waic to file
    cat(c(i, waic, seeds[i], unlist(combs[i,]), '\n'), file= "../results/waic_SAmRev.txt",append=TRUE)
    
    # save the model
    save(model, file = paste0("../models/SAmRev/", toString(i), ".Rda"))
