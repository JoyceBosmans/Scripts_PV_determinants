...
rlimit_as(10e12,10e12)
print('NOTE: currently running with ~1/10th of df_full!')

### set output file names etc
df_SAm_allgridcells				<- 'df_SAm_allgridcells.csv'
outputfilename_raster           <- 'Probability_map_SAm.gri'
outputfilename_raster_nocountry <- 'Probability_map_SAm_fixed.gri'
best_model                      <- '1138'
...
if(file.exists(df_SAm_allgridcells)) {
  print('loading dataframe with all grid cells')
  df_full <- read.csv(df_SAm_allgridcells)
} else {
  print('updating dataframe with grid_km and storing in this dir')
  ### dataframe with all grid cells already created in ../PV_determinants_SAm/Script_create_probability_maps.R
  df_full <- read.csv(file.path('../PV_determinants_SAm/df_SAm_allgridcells_updated_gridkm.csv'))

  ### add Chile (mistakingly excluded thus far)
  df_Chile <- read.csv('df_Chile_allgridcells.csv')
  df_Chile$lat_orig  <- df_Chile$y
  df_Chile$lon_orig  <- df_Chile$x
  df_full            <- rbind.fill(df_full,df_Chile)
  ...
  fwrite(df_full,df_SAm_allgridcells)
}
...
#### plot
load('tab_coef_SAm.Rda')	# model coefficients

df_full <- df_full[complete.cases(df_full$travel_log),]	

predict_fixed     <- tab_coef_SAm['intercept',] + tab_coef_SAm['travel_log',]*df_full$travel_log + tab_coef_SAm['slope_log',]*df_full$slope_log + tab_coef_SAm['road_log',]*df_full$road_log + tab_coef_SAm['grid_log',]*df_full$grid_log + tab_coef_SAm['rsds',]*df_full$rsds
probability_fixed <- 1/(1 + exp(-predict_fixed))
df_full$probability_fixed <- probability_fixed
...
plot_SAm_fixed   <- ggplot() + geom_tile(data=df_full, aes(x=lon_orig, y=lat_orig, fill=probability_fixed), alpha=0.8) +  theme(
...
pdf(file='Plot_probability_SAm_fixed.pdf')
plot_SAm_fixed
dev.off()

sapply(df_full, function(y) sum(length(which(is.na(y)))))
