...
formula <- y ~ 1 + travel_log + slope_log + road_log + grid_log + rsds + f(SOVEREIGN1, model = "iid") + f(spatial.field, model = spde)
...
sign                <- if(sign(tab_coef_SAm['travel_log',]) < 0) '#cc3300' else '#3399FF'
...
sign                <- if(sign(tab_coef_SAm['slope_log',]) < 0) '#cc3300' else '#3399FF'	
...
print(paste('pR2:',pseudoRsquared,'pR2 random road:',pseudoRsquared_pred,'PI road:',PI_road_log))

sign                <- if(sign(tab_coef_SAm['road_log',]) < 0) '#cc3300' else '#3399FF'	
...
sign                <- if(sign(tab_coef_SAm['grid_log',]) < 0) '#cc3300' else '#3399FF'	
...
sign                <- if(sign(tab_coef_SAm['rsds',]) < 0) '#cc3300' else '#3399FF'	
...
df_PI
write_xlsx(df_PI,'df_PI_SAm.xlsx')
