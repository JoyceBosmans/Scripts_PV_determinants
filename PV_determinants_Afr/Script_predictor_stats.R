...
df_PA <- read_xlsx('df_PA_Afr_country.xlsx')
...
print('NOTE: grid now in km!')
df_PA$grid_proximity <- df_PA$grid_km 
df_PA$travel_time    <- df_PA$access
df_PA$irradiance     <- df_PA$rsds
...
# vifs with all predictors give NA (due to additive nature of land cover fractions). 
# removing either forest or agriculture leads to very similar vifs. Continuing without forest.
...
# --> log_transform slope, grid, travel_time and road_dist (elevation as well if also done for other continents)
