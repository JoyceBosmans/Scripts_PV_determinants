...
print('   and save')
names(pred_stack_global) <- c('access')
writeRaster(pred_stack_global,filename='pred_stack_access.gri',overwrite=TRUE,format='raster')
...
pred_stack_orig  <- dropLayer(pred_stack_orig,'access')
...
names(pred_stack) <- c('road_dist','urban_dist','grid','slope','elev','protect','rsds','wind','access')
