...
df_Asia_allgridcells			<- 'df_Asia_allgridcells_1.csv'
...
if(file.exists(df_Asia_allgridcells)) {
  ...
  print(paste('NOTE: half of df_full!! creating',df_Asia_allgridcells))
  df_full <- df_full[1:23744998,]
  ...
  df_full            <- subset(df_full,select=c('grid','protect','access','rsds','road_dist','slope','elev','Agriculture','Forest','Short_natural','Wetland','Urban','Bare','Water','available_area','layer','lat_orig','lon_orig','SOVEREIGN1','grid_km','Y_1.y','x_1.y','Y_1.x','x_1.x'))
  ...
  print(paste('NOTE: half of df_full!! Created',df_Asia_allgridcells, 'of length',nrow(df_full)))
}
