...
df_PA_orig <- read_xlsx('../PV_determinants_Asia/df_PA_Asia_country_060622.xlsx')
...
# set random seed    
set.seed(model_performance[which.min(model_performance$V2), 3])
...
# get the predictions without country effect
...
formula <- y ~ 1 + travel_log + slope_log + elev_log + road_log + grid_log + protect + rsds + Agriculture + Short_natural + Wetland + Urban + Bare + Water + f(SOVEREIGN1, model = "iid") + f(spatial.field, model = spde)
...
write_xlsx(df_scores,'df_scores_Asia.xlsx')
...
write_xlsx(df_scores_nocountry,'df_scores_nocountry_Asia.xlsx')
