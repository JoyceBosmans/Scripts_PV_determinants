...
df_PA          <- read_xlsx('df_PA_SAm_country_261121.xlsx')
#~ outputfilename <- 'df_PI_SAm.Rda'
outputfilename_csv <- 'df_PI_SAm.csv'
...
# average model created in Script_find_best_glmer.R (average of all models within 2 AIC of best model): 
load('avg_model.rda')
...
# get prediction of full model-averaged coefficients
...
# randomize each predictor and determine R. Travel time (a priori), Wetland and Urban are excluded for NAm. 
...
sign              <- if(coefft$elev_log[1] < 0) '#cc3300' else '#3399FF'
...
df_random <- transform(df_scaled, Short_natural = sample(Bare,replace=FALSE))
random_prediction <- predict(avg_model, newdata = df_random, type = "response",allow.new.levels=TRUE)
PI                <- 1 - cor(best_prediction,random_prediction,method='spearman')
sign              <- if(coefft$Short_natural[1] < 0) '#cc3300' else '#3399FF'
df_PI_add         <- data.frame('Short natural',PI,sign)
names(df_PI_add)  <- c('predictor','PI','sign')
df_PI             <- rbind(df_PI,df_PI_add)

df_random <- transform(df_scaled, Urban = sample(Urban,replace=FALSE))
random_prediction <- predict(avg_model, newdata = df_random, type = "response",allow.new.levels=TRUE)
PI                <- 1 - cor(best_prediction,random_prediction,method='spearman')
sign              <- if(coefft$Urban[1] < 0) '#cc3300' else '#3399FF'
df_PI_add         <- data.frame('Urban',PI,sign)
names(df_PI_add)  <- c('predictor','PI','sign')
df_PI             <- rbind(df_PI,df_PI_add)
